...
- default to the C calling convention (`__cdecl` on MSVC);
...
The initial public reading type enum mirrors the categories already parsed in
`sensorwatch.hwinfo_shm.SENSOR_TYPES`:
