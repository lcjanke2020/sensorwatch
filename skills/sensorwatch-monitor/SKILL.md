...
pass: **one** summary `report`; the deterministic **logger-liveness** check from
that report's `meta` block (zero-sample digest = logger dead / machine off;
large `meta.last_sample` lag or a trailing `gaps` entry = feed stalled — see the
sensorwatch skill, Recipe 4); a review of open incidents (any past snooze?); and,
on the **first heartbeat after midnight**, the [maintenance pass](#maintenance-pass).
Then re-arm.

**On watcher-health failure.** If `watch` dies fatally (exit `1`): re-arm with
**backoff**, and record `consecutive_watch_failures` in `heartbeat.json`. After
**three consecutive failures**, open a **monitoring-blind incident** and escalate
— *a blind monitor is itself reportable.* On a **config error (exit 2): STOP and
surface it — never re-arm blindly** (a hot-loop on bad `[[rules]]` helps no one).
Exit `130` is a signalled shutdown.
...
`escalation_gate.py` is the **deterministic** decision — a pure function of the
ledger plus the event's shape. It is check-then-commit in one invocation
(`--commit`), so a fresh session can never re-notify. Tiers:
