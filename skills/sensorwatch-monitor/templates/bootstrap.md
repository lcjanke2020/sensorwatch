...
`notify.toml` is present). Tier 3 adds `--issue-draft`: the same invocation
also writes a tracker-ready draft to `outbox/issues/` (an artifact, not a
delivery — the cooldown/daily count records once). Drafts and notification
bodies are plain prose referencing incident-file paths and never embed sensor
data.
